...
function generateCloud() {
  ...
  cloudGroup.applyMatrix4(
    new THREE.Matrix4().makeRotationY(Math.PI * 2 * (Math.random() * 10))
  );
...
airPlaneGroup.scale.set(0.35, 0.35, 0.35), scene.add(airPlaneGroup);
...
function onWindowResize() {
  ...
  document
    .querySelectorAll("#webProject")
    [currentWebSlide - 1].classList.remove("is-in");
  document
    .querySelectorAll("#dotWeb")
    [currentWebSlide - 1].classList.remove("is-at");
  document
    .querySelectorAll("#gameProject")
    [currentGameSlide - 1].classList.remove("is-in");
  document
    .querySelectorAll("#dotGame")
    [currentGameSlide - 1].classList.remove("is-at");
  webSlidePos = 0;
  ...
  document
    .querySelectorAll(".imageWrap")
    .forEach((each) => (each.style.translate = 0));
  document
    .querySelectorAll(".dotImg")
    .forEach((each) => each.classList.remove("is-focus"));
  document
    .querySelectorAll(".dotGroup")
    .forEach((each) => each.children[0].classList.add("is-focus"));
  document
    .querySelectorAll(".projectSecret")
    .forEach((each) => (each.style.width = "1px"));
}
...
function onMouseMove(event) {
  ...
  airPlaneGroup.rotation.z = airPlaneGroup.rotation.x =
    airPlaneNewPos.y - airPlaneGroup.position.y;
}
...
function onTouchMove(event) {
  ...
  //Update airplane
  airPlaneNewPos.x =
    (event.changedTouches[0].clientX * 3) / window.innerWidth - 1.5;
  airPlaneNewPos.y =
    -((event.changedTouches[0].clientY * 1.6) / window.innerHeight) - 4.2;
  airPlaneGroup.rotation.z = airPlaneGroup.rotation.x =
    airPlaneNewPos.y - airPlaneGroup.position.y;
}
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    if (currentWebSlide === totalWebSlide) {
      ...
      webSlidePos =
        webSlidePos -
        document.querySelector("#webWrapper").getBoundingClientRect().width /
          totalWebSlide;
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    if (currentGameSlide === totalGameSlide) {
      ...
      gameSlidePos =
        gameSlidePos -
        document.querySelector("#gameWrapper").getBoundingClientRect().width /
          totalGameSlide;
...
document.querySelectorAll("#nextImg").forEach((each) => {
  each.addEventListener("click", function (event) {
    ...
    whoosh.play();
    const amount =
      this.closest(".project").children[2].children[1].children.length;
    const currentImage =
      this.closest(".project").children[3].getBoundingClientRect().width;
    if (currentImage < amount) {
      const step =
        this.closest(".project").children[1].getBoundingClientRect().width *
        currentImage;
      this.closest(".project").children[1].children[0].style.translate =
        "-" + step + "px";
      this.closest(".project").children[2].children[1].children[
        currentImage - 1
      ].classList.remove("is-focus");
      this.closest(".project").children[2].children[1].children[
        currentImage
      ].classList.add("is-focus");
      this.closest(".project").children[3].style.width =
        currentImage + 1 + "px";
    } else {
      ...
      this.closest(
        ".project"
      ).children[2].children[1].children[0].classList.add("is-focus");
...
function projectResize() {
  if (window.innerWidth > 820) {
    if (window.innerHeight / window.innerWidth >= 0.9) {
      ...
      document.querySelector("#prevWebButton").style.transform =
        "translateX(0)";
      document.querySelector("#nextWebButton").style.transform =
        "translateX(0)";
      document.querySelector("#prevGameButton").style.transform =
        "translateX(0)";
      document.querySelector("#nextGameButton").style.transform =
        "translateX(0)";
      document
        .querySelectorAll(".picture")
        .forEach((each) => (each.style.width = "65vw"));
    } else {
      document.querySelectorAll(".project").forEach((each) => {
        each.style.width = "30vw";
      });
      document.querySelector("#prevWebButton").style.transform =
        "translateX(calc(20vw - 30px))";
      document.querySelector("#nextWebButton").style.transform =
        "translateX(calc(-20vw + 30px))";
      document.querySelector("#prevGameButton").style.transform =
        "translateX(calc(20vw - 30px))";
      ...
      document
        .querySelectorAll(".picture")
        .forEach((each) => (each.style.width = "30vw"));
    }
  }
}
...
const animate = () => {
  ...
  document
    .querySelectorAll("#dotWeb")
    [currentWebSlide - 1].classList.add("is-at");
  document
    .querySelectorAll("#webProject")
    [currentWebSlide - 1].classList.add("is-in");

  //Update game project slide
  document.querySelector("#gameWrapper").style.translate = gameSlidePos + "px";
  document
    .querySelectorAll("#dotGame")
    [currentGameSlide - 1].classList.add("is-at");
  document
    .querySelectorAll("#gameProject")
    [currentGameSlide - 1].classList.add("is-in");
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const startTypewriter = () => {
    setTimeout(type, 500); // Adjust the delay if needed
  };

  // Check if the typewriter element is in view
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        startTypewriter();
        observer.disconnect(); // Stop observing once the effect has started
      }
    });
  });
...
let pauseBetweenWords = 2500; // 2.5 seconds pause when the full text is displayed
let pauseBeforeTyping = 500; // Pause before typing starts
